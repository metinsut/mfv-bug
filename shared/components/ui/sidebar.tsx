      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ...
            : "group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[collapsible=icon]:group-data-[side=left]:border-e group-data-[side=right]:border-s",
          className,
        )}
        {...props}
      >
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      {open ? (
        <CaretDoubleLeftIcon size={16} className="rtl:rotate-180" />
      ) : (
        <CaretDoubleRightIcon size={16} className="rtl:rotate-180" />
      )}
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-sidebar md:peer-data-[variant=inset]:m-2",
        "md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:rounded-xl",
        "md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-2",
        "shadow-none",
        className,
      )}
      {...props}
...
const sidebarMenuButtonVariants = cva(
  cn(
    ...
    "rounded-[calc(var(--radius-sm)+2px)] p-2 text-start ring-sidebar-ring",
    "outline-hidden transition-[width,height,padding,color,background-color] group-has-data-[sidebar=menu-action]/menu-item:pe-8",
  ...
  ),
  {
    variants: {
      ...
      size: {
        default: "h-8 text-sm font-500",
        sm: "h-7 text-xs",
        lg: "h-12 text-sm font-500 group-data-[collapsible=icon]:p-0!",
      },
    },
    defaultVariants: {
      variant: "default",
